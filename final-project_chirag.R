...
set.seed(1) 
row.number <- sample(x=1:nrow(x_data), size=0.8*nrow(x_data))
...
randomforest.model <- randomForest(as.factor(Exited) ~ ., data = train, importance = TRUE)
randomforest.model
...
randomforest.model_learnt_mtry2 <- randomForest(as.factor(Exited) ~ ., data = train, ntree = 200, mtry = 2, importance = TRUE)
randomforest.model_learnt_mtry2
#as negative var expalined so model is overfitted

randomforest.model_learnt_mtry4 <- randomForest(Exited ~ ., data = train, ntree = 200, mtry = 4, importance = TRUE)
randomforest.model_learnt_mtry4

randomforest.model_learnt_mtry5 <- randomForest(Exited ~ ., data = train, ntree = 200, mtry = 5, importance = TRUE)
...
predicted_table <- table(observed=test$Exited,predicted=predTest)
...
plot( predict(randomforest.model_learnt_mtry2), train$Exited)
abline(c(0,1),col=3)
...
churn_gbm.model=gbm(Exited ~ . ,data = train,distribution = "gaussian",n.trees = 5000,
...
cor(train$CreditScore,train$Exited)#negetive correlation coeff-r
cor(train$Balance,train$Exited)#positive correlation coeff-r
...
test.error<-with(test,apply( (predmatrix_gbm-Exited)^2,2,mean))
